...
//		if (strlen($x) != 32 || strlen($y) != 32) { $this->oops('cannot decode key response (15)'); }
		$keyBytes = chr(4).$x.$y;
		//$rawId = array_to_string($info->rawId);
...
//		$publicKey->id = $info->rawId;
		...
		insert(base64url_encode($credId), base64url_encode($keyBytes), "mane");
		//echo $attestion;
...
$(document).ready(function(){
		...
		var ao = [];
		(new Uint8Array(newCredential.response.attestationObject)).forEach(function(v){ ao.push(v); });	
